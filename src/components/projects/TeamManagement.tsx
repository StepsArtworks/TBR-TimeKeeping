...
export function TeamManagement({
  ...
  // Use live query for available users
  const availableUsers = useLiveQuery(
    async () => {
      try {
        // Get all users except those already in the team
        const allUsers = await db.users
          .where('role')
          .notEqual('admin') // Exclude admin users
          .toArray();
        
        return allUsers.filter(user => !members.find(member => member.id === user.id));
      } catch (err) {
